...
def radix_sort(array):
    if not array:                                           #stop the process if array is empty
        return array
    offset = abs(min(array)) if min(array) < 0 else 0       #shift all the value of the list by the min number's value to solve the negative number
    array = [ num + offset for num in array]
    ...
    array = [ num - offset for num in array]                #shifting all the value of the list back to original values
    return array
